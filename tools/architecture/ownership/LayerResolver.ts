...
export class LayerResolver {
  static resolve(relativePath: string, fileName: string): Layer {
    ...
    if (fileName === 'page.tsx' || fileName === 'page.ts' || fileName === 'page.js' || fileName === 'page.jsx') return Layer.Page;
    if (fileName === 'layout.tsx' || fileName === 'layout.ts') return Layer.Layout;
    if (fileName === 'route.ts' || fileName === 'route.tsx') return Layer.Route;
    if (fileName === 'loading.tsx' || fileName === 'error.tsx' || fileName === 'not-found.tsx') return Layer.Page;
    
    if (p.includes('/components/') || fileName.endsWith('.tsx') || fileName.endsWith('.jsx')) return Layer.Component;
    ...
    if (p.includes('/validators/') || fileName.includes('validator') || fileName.includes('schema') || p.includes('zod')) return Layer.Validator;
    ...
    if (p.includes('/utils/') || p.includes('/lib/') || fileName.includes('util') || fileName.includes('helper')) return Layer.Utility;
    if (p.includes('/types/') || fileName.endsWith('.d.ts') || fileName.includes('type')) return Layer.Type;
    ...
    if (p.includes('config') || fileName.endsWith('config.ts') || fileName.endsWith('config.mjs') || fileName.endsWith('.json')) return Layer.Configuration;
    if (p.includes('scripts/') || fileName.endsWith('.sh') || p.includes('/tools/')) return Layer.Script;
