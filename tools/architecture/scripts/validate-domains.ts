...
async function validate() {
  ...
  for (const sf of sourceFiles) {
    ...
    if (sf.getEndLineNumber() > 1000) {
      ...
      violations.push({
        file: filePath,
        type: "OVERSIZED",
        message: `File exceeds 1000 lines (${sf.getEndLineNumber()} lines).`,
      });
    }
    ...
    for (const imp of imports) {
      ...
      if (isDeep)
        violations.push({
          file: filePath,
          type: "DEEP_IMPORT",
          message: `Deep import into '${targetDomain}'.`,
        });
      if (sourceDomain === "shared" && isTargetBusiness)
        violations.push({
          file: filePath,
          type: "SHARED_VIOLATION",
          message: `Shared cannot import business domain '${targetDomain}'`,
        });
      if (sourceDomain === "platform" && isTargetBusiness)
        violations.push({
          file: filePath,
          type: "PLATFORM_VIOLATION",
          message: `Platform cannot import business domain '${targetDomain}'`,
        });
      if (isSourceBusiness && isTargetBusiness)
        violations.push({
          file: filePath,
          type: "CROSS_DOMAIN",
          message: `Cross-domain import from '${sourceDomain}' to '${targetDomain}'.`,
        });
    }
  }
  ...
  for (const e of bootGraphEdges)
    bootGraphMd += `  ${e.from.replace(/[-/]/g, "_")} --> ${e.to.replace(/[-/]/g, "_")}\n`;
  bootGraphMd += "```\n";
  ...
  fs.writeFileSync(
    path.join(REPORTS_DIR, "architecture-health.md"),
    `# Architecture Health\nScore: ${score}/100\nViolations: ${violations.length}`
  );
  fs.writeFileSync(
    path.join(REPORTS_DIR, "boundary-violations.json"),
