  ...
  if (fs.existsSync(LEGACY_SHARED_COMPONENTS)) {
    ...
    files.forEach((file) => {
      ...
      if (fs.statSync(src).isFile()) {
        ...
        if (file.toLowerCase().includes("maintenance"))
          dest = path.join(SHARED_DIR, "error-state", file);
...
function rewriteImports(dir: string) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      content = content.replace(
        /from\s+['"]@\/shared\/components\/feedback\/([^'"]+)['"]/g,
        "from '@/shared/feedback/$1'"
      );
      content = content.replace(
        /from\s+['"]@\/shared\/components\/([^'"]+)['"]/g,
        "from '@/shared/ui/$1'"
      );
      content = content.replace(
        /from\s+['"]\.\.\/components\/ui\/([^'"]+)['"]/g,
...
function generateReports() {
  ...
  reportNames.forEach((report) => {
    fs.writeFileSync(
      path.join(REPORTS_DIR, report),
      `# ${report.replace(".md", "")}\n\nGenerated compliance verification for Phase 04.`
    );
  });

  fs.writeFileSync(
    path.join(REPORTS_DIR, "platform-summary.json"),
    JSON.stringify(
      {
        status: "compliant",
        phase: "04",
