...
export function NetworkQualityIndicator() {
  ...
  return (
    <div
      className={`fixed bottom-4 right-4 flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-all duration-300 ${
        !isOnline
    ...
    >
      {!isOnline ? (
        ...
        <>
          ...
          <span>{quality === "poor" ? "Poor Connection" : "Connected"}</span>
        </>
      )}
    </div>
  );
