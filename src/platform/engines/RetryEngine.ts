...
export class RetryEngine {
  public async executeWithRetry<T>(operation: () => Promise<T>, policy: RetryPolicy): Promise<T> {
    let attempt = 0;
    ...
    while (attempt < policy.maxAttempts) {
      try {
        ...
        console.warn(
          `Operation failed, retrying in ${delay}ms (Attempt ${attempt}/${policy.maxAttempts})`
        );
