...
import { QuestionRestored } from "../events/QuestionEvents";

export class RestoreQuestionWorkflow {
  constructor(
  ...
  public async execute(questionId: string, userId: string, tx: TransactionClient): Promise<void> {
    ...
    await this.questionRepo.restoreQuestion(questionId, tx);

    // If there's an active published revision that was archived, restore it
    if (question.currentRevisionId) {
      await this.revisionRepo.updateRevisionStatus(
        question.currentRevisionId,
        RevisionStatus.PUBLISHED as any,
        tx
      );
    }
    ...
    // Emit Domain Event
    if (this.eventBus) {
      await this.eventBus.publish(new QuestionRestored(questionId, userId));
    }
  }
}
