  ...
  return (
    <div className="p-3 border border-gray-100 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/50">
      <div className="flex justify-between items-start mb-1">
        <span
          className={`text-xs font-semibold px-2 py-0.5 rounded ${isCritical ? "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400" : "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400"}`}
        >
          {severity}
        </span>
      ...
      <div className="mt-2 flex items-center justify-between">
        <span
          className={`text-xs font-medium flex items-center gap-1 ${isRecovered ? "text-green-600 dark:text-green-400" : "text-blue-600 dark:text-blue-400"}`}
        >
          <div
            className={`w-1.5 h-1.5 rounded-full ${isRecovered ? "bg-green-500" : "bg-blue-500"}`}
          ></div>
          {status}
        </span>
        {!isRecovered && (
