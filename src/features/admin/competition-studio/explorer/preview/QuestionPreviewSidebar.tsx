    ...
    return (
      <aside className="w-96 border-l border-gray-200 bg-gray-50 hidden xl:flex flex-col shrink-0">
        ...
        <div className="flex-1 p-8 text-center flex flex-col items-center justify-center text-gray-500">
          ...
          <p className="text-sm">Select a question from the grid to see detailed preview, analytics, and health metrics here.</p>
        </div>
      </aside>
    );
  }
    ...
    <aside className="w-96 border-l border-gray-200 bg-white hidden xl:flex flex-col shrink-0 shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-10 relative">
      ...
      {isLoading || !questionDetail ? (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* Question Body */}
          <div className="p-5">
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Prompt</h4>
            <div className="text-sm text-gray-800 leading-relaxed bg-gray-50 p-4 rounded-lg border border-gray-100">
              {questionDetail.text}
            </div>
            
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mt-6 mb-2">Options</h4>
            <div className="space-y-2">
              {questionDetail.options.map((opt: string, idx: number) => (
                ...
                >
                  <span className="inline-block w-6 text-gray-400 font-bold">{String.fromCharCode(65 + idx)}.</span> {opt}
                </div>
              ))}
            </div>
            
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mt-6 mb-2">Explanation</h4>
          ...
          <div className="p-5 border-t border-gray-100 bg-gray-50">
             <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Usage Analytics</h4>
             <div className="grid grid-cols-3 gap-3">
               ...
               <div className="bg-white p-3 rounded shadow-sm border border-gray-200 text-center">
                 ...
                 <div className="font-bold text-green-600">{questionDetail.metadata.successRate}</div>
               </div>
               <div className="bg-white p-3 rounded shadow-sm border border-gray-200 text-center">
