...
export class SemanticDifferenceEngineService {
   ...
   */
  compare(oldVersion: CompetitionVersionArtifact, newVersion: CompetitionVersionArtifact): string[] {
    const changes: string[] = [];
    
    // Metadata diff
    if (oldVersion.competitionSnapshot.title !== newVersion.competitionSnapshot.title) {
      changes.push(`Title changed from "${oldVersion.competitionSnapshot.title}" to "${newVersion.competitionSnapshot.title}"`);
    }

    // Configuration diff
    if (oldVersion.rulesSnapshot.strictMode !== newVersion.rulesSnapshot.strictMode) {
      changes.push(`Strict mode changed: ${oldVersion.rulesSnapshot.strictMode} -> ${newVersion.rulesSnapshot.strictMode}`);
    }

    // Sections diff
    if (oldVersion.sectionsSnapshot.length !== newVersion.sectionsSnapshot.length) {
      changes.push(`Section count changed: ${oldVersion.sectionsSnapshot.length} -> ${newVersion.sectionsSnapshot.length}`);
    }

    // Questions diff
    if (oldVersion.questionsSnapshot.length !== newVersion.questionsSnapshot.length) {
      changes.push(`Question count changed: ${oldVersion.questionsSnapshot.length} -> ${newVersion.questionsSnapshot.length}`);
    }
