...
export class SchemaCompatibilityEngine {
   ...
   */
  async upcastArtifactIfNecessary(artifact: CompetitionVersionArtifact): Promise<CompetitionVersionArtifact> {
    ...
    if (artifact.schemaVersion < CURRENT_SCHEMA_VERSION) {
      console.warn(`Upcasting artifact from schema ${artifact.schemaVersion} to ${CURRENT_SCHEMA_VERSION}`);
