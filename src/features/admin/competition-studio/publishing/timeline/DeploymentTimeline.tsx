...
export function DeploymentTimeline() {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800">
      <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Deployment Timeline</h3>
        ...
        {states.map((state, index) => {
          ...
          return (
            <div key={state} className="mb-6 ml-6 relative">
              <span className={`absolute flex items-center justify-center w-6 h-6 rounded-full -left-[35px] ring-4 ring-white dark:ring-gray-900 
              ...
              <h4 className={`text-sm font-semibold tracking-wide ${isCurrent ? 'text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-400'}`}>
                {state.replace('_', ' ')}
              </h4>
              
              {isCurrent && state === DeploymentStatus.EXECUTING && currentStage && (
                <p className="text-xs text-gray-500 mt-1">
                  Executing Stage: <span className="font-medium text-gray-800 dark:text-gray-200">{currentStage}</span>
                </p>
              )}
            </div>
          );
