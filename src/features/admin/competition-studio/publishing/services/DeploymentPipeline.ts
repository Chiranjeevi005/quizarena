import { PrismaClient, DeploymentStatus, DeploymentStrategy, DeploymentEnvironment, DeploymentReason, DeploymentResult } from '@/generated/prisma';
...
export class DeploymentPipeline {
  ...
  public async planDeployment(options: PublishDeploymentOptions) {
    // 1. Acquire Lock
    await lockService.acquireLock(options.competitionId, options.initiatedById, 'Deployment Planning');
    ...
    try {
      ...
      // 3. Generate Plan
      const plan = await planner.generatePlan(options.versionId, options.environment, options.strategy);
      ...
      const deploymentNumber = await prisma.publishDeployment.count({
