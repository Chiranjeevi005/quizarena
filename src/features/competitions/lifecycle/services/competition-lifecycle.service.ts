...
export class CompetitionLifecycleService {
  ...
  private async transition(
    ...
    if (
      previousState === CompetitionLifecycle.LIVE &&
      newState !== CompetitionLifecycle.COMPLETED &&
      newState !== CompetitionLifecycle.PAUSED &&
      newState !== CompetitionLifecycle.CANCELLED &&
      newState !== CompetitionLifecycle.EXPIRED
    ) {
  ...
  async moveToReady(
    competitionId: string,
    actorId: string,
    actorType: LifecycleActorType = LifecycleActorType.ADMIN
  ) {
    const readiness = await competitionReadiness.evaluateReadiness(competitionId);
    if (!readiness.isReady) {
      throw new Error(
        `Cannot transition to READY. Readiness checks failed: ${readiness.errors.join(", ")}`
      );
    }
  ...
  async emergencyPause(
    ...
    reason: string,
    actorId: string,
    actorType: LifecycleActorType = LifecycleActorType.SUPER_ADMIN
  ) {
    await this.transition(
      competitionId,
  ...
  private async validateLifecycleTransition(
    competitionId: string,
  ...
  private async executeBeforeHooks(
    oldState: CompetitionLifecycle,
    newState: CompetitionLifecycle,
    competitionId: string
  ) {
    if (newState === CompetitionLifecycle.READY)
      await competitionHooks.executeHooks("beforeReady", competitionId);
    if (newState === CompetitionLifecycle.SCHEDULED)
      await competitionHooks.executeHooks("beforeScheduled", competitionId);
    if (newState === CompetitionLifecycle.LIVE)
      await competitionHooks.executeHooks("beforeLive", competitionId);
    if (newState === CompetitionLifecycle.COMPLETED)
      await competitionHooks.executeHooks("beforeCompleted", competitionId);
    if (newState === CompetitionLifecycle.ARCHIVED)
      await competitionHooks.executeHooks("beforeArchived", competitionId);
  }
  ...
  private async executeAfterHooks(
    ...
    if (newState === CompetitionLifecycle.READY)
      await competitionHooks.executeHooks("afterReady", competitionId);
    if (newState === CompetitionLifecycle.SCHEDULED)
      await competitionHooks.executeHooks("afterScheduled", competitionId);
    if (newState === CompetitionLifecycle.LIVE)
      await competitionHooks.executeHooks("afterLive", competitionId);
    if (newState === CompetitionLifecycle.COMPLETED)
      await competitionHooks.executeHooks("afterCompleted", competitionId);
    if (newState === CompetitionLifecycle.ARCHIVED)
      await competitionHooks.executeHooks("afterArchived", competitionId);
  }
