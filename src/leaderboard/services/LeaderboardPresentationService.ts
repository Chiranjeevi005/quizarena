...
} from "../repositories/RankingSnapshotRepository";
...
export class LeaderboardPresentationService {
  ...
  public async getStatistics(
    competitionId: string,
    scope: string
  ): Promise<LeaderboardStatistics | null> {
    const aggregate = await this.aggregateRepo.findLatestByCompetitionId(competitionId, scope);
    if (!aggregate) return null;
  ...
  public async getCurrentUser(
    ...
    scope: string,
    userId: string
  ): Promise<LeaderboardCurrentUser | null> {
    const aggregate = await this.aggregateRepo.findLatestByCompetitionId(competitionId, scope);
    if (!aggregate) return null;
  ...
  public async getEntries(
    ...
    limit: number,
    cursor?: string
  ): Promise<CursorPaginationResult<LeaderboardEntry>> {
    const aggregate = await this.aggregateRepo.findLatestByCompetitionId(competitionId, scope);
    if (!aggregate) {
      return { data: [], nextCursor: null, prevCursor: null, hasMore: false, totalCount: 0 };
    }

    const paginatedSnapshots = await this.snapshotRepo.findByAggregateIdWithCursor(
