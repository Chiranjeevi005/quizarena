    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-gray-500">Real-time operational metrics and system alerts.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border bg-white text-card-foreground shadow-sm p-6">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Avg delay: {data.webhookDelayMs}ms
          </p>
        </div>
        ...
        <div className="rounded-xl border bg-white text-card-foreground shadow-sm p-6">
          ...
          <ul className="space-y-4">
            <li className="flex justify-between items-center text-sm">
              ...
              <span className="font-medium">{new Date(data.lastSuccessfulPaymentAt).toLocaleString()}</span>
            </li>
            <li className="flex justify-between items-center text-sm">
              <span className="text-gray-500">Successful Refund</span>
              <span className="font-medium">{data.lastSuccessfulRefundAt ? new Date(data.lastSuccessfulRefundAt).toLocaleString() : 'N/A'}</span>
            </li>
            <li className="flex justify-between items-center text-sm">
              <span className="text-gray-500">Settlement Synced</span>
              <span className="font-medium">{data.lastSettlementAt ? new Date(data.lastSettlementAt).toLocaleString() : 'N/A'}</span>
            </li>
          </ul>
        </div>
