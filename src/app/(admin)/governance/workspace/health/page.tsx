      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-3xl font-bold ${overallScore >= 99 ? "text-green-600" : "text-yellow-600"}`}
            >
              {overallScore}%
            </span>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {healthMetrics.map((metric) => (
            <div
              key={metric.name}
              className="flex flex-col items-center justify-center p-4 bg-gray-50 rounded-lg border border-gray-100"
            >
              <span className="text-sm font-medium text-gray-500 mb-2">{metric.name}</span>
              <span
                className={`text-xl font-semibold ${metric.score === 100 ? "text-green-600" : "text-yellow-600"}`}
              >
                {metric.score}%
              </span>
            </div>
