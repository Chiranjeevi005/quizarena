...
export default async function PracticeArenaPage() {
  ...
  if (intelligence.weakness && intelligence.weakness.weakestCategory) {
    recommendedChallenge =
      activeChallenges.find((c) => c.category === intelligence.weakness!.weakestCategory) ||
      latestChallenge;
  } else if (hasHistory && performance?.weakestCategory) {
    recommendedChallenge =
      activeChallenges.find((c) => c.category === performance.weakestCategory) || latestChallenge;
  } else {
  ...
  const todayAvgScore =
    todayAttempts.length > 0 ? Math.round(todayScore / todayAttempts.length) : 0;
  ...
  if (hasHistory) {
    if (intelligence.weakness) {
      trainingPlan.push({
        category: intelligence.weakness.weakestCategory,
        label: "Core Weakness",
      });
    }
    intelligence.actions.forEach((action) => {
      if (trainingPlan.length < 3 && !trainingPlan.find((p) => p.category === action.category)) {
        trainingPlan.push({
          ...
          label: action.type === "speed" ? "Speed Drills" : "Practice",
        });
      }
    });
          ...
          <div>
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-emerald-500/20 text-emerald-300 text-xs font-bold rounded-md uppercase tracking-widest">
                {intelligence.maturityLevel === 0
                  ? "Beginner"
                  : intelligence.maturityLevel === 1
                    ? "Novice"
                    : "Intermediate"}
              </div>
            </div>
              ...
              <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-8 backdrop-blur-sm">
                  ...
                  <span className="px-2.5 py-1 bg-primary/20 text-blue-300 text-xs font-bold rounded-md uppercase tracking-wider">
                    {recommendedChallenge.category
                      ? EXAM_CATEGORY_LABELS[
                          recommendedChallenge.category as keyof typeof EXAM_CATEGORY_LABELS
                        ] || recommendedChallenge.category
                      : "Balanced Practice"}
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-5 text-sm text-gray-400">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {recommendedChallenge.durationInMinutes} Minutes
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-blue-400" />{" "}
                    {recommendedChallenge.totalQuestions} Questions
                  </div>
                </div>
              </div>
            )}
            ...
            {!hasHistory && (
              <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-8 backdrop-blur-sm flex items-start gap-3">
                ...
                <p className="text-sm font-medium text-gray-300 leading-relaxed">
                  <strong>Training Momentum:</strong> Complete your first session to activate
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              {recommendedChallenge ? (
                <RecommendedChallengeCTA
                  challenge={recommendedChallenge}
                  label={hasHistory ? "Begin Training Session" : "Start Assessment Session"}
                />
              ) : (
                <button
                  disabled
                  className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-gray-700 text-gray-400 h-12 px-8 rounded-xl font-bold"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <>
                    <div className="font-bold text-gray-900 truncate">
                      {performance?.strongestCategory
                        ? EXAM_CATEGORY_LABELS[
                            performance.strongestCategory as keyof typeof EXAM_CATEGORY_LABELS
                          ] || performance.strongestCategory
                        : "—"}
                    </div>
                    <div className="text-2xl font-black text-gray-900 mt-1">
                      {intelligence.categories.find(
                        (c) => c.category === performance?.strongestCategory
                      )?.accuracy || 0}
                      %
                    </div>
                  </>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm border-l-4 border-l-red-500">
                ...
                {hasHistory ? (
                  <>
                    <div className="font-bold text-gray-900 truncate">
                      {intelligence.weakness?.weakestCategory
                        ? EXAM_CATEGORY_LABELS[
                            intelligence.weakness
                              .weakestCategory as keyof typeof EXAM_CATEGORY_LABELS
                          ] || intelligence.weakness.weakestCategory
                    ...
                    <div className="text-2xl font-black text-gray-900 mt-1">
                      {intelligence.weakness?.categoryAccuracy ||
                        intelligence.categories.find(
                          (c) => c.category === performance?.weakestCategory
                        )?.accuracy ||
                        0}
                      %
                    </div>
                    ...
                    {trainingPlan.map((step, idx) => {
                      ...
                      return (
                        <div key={idx} className="relative flex items-center gap-6">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center relative z-10 shrink-0 border-2 ${
                              isCompleted
                          ...
                          <div className="flex-1 bg-gray-50 border border-gray-100 rounded-lg p-4 flex justify-between items-center">
                            <div>
                              ...
                              >
                                {EXAM_CATEGORY_LABELS[
                                  step.category as keyof typeof EXAM_CATEGORY_LABELS
                                ] || step.category}
                              </div>
                            </div>
                            <div
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-xs font-medium text-gray-500">
                          <span className="flex items-center gap-1 text-emerald-600">
                            <CheckCircle2 className="w-3.5 h-3.5" /> {attempt.accuracy}% Acc
                          </span>
                          <span>
                            {attempt.submittedAt
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 relative overflow-hidden">
              ...
              {intelligence.actions && intelligence.actions.length > 0 && hasHistory ? (
                <div className="relative z-10">
                  <div className="text-sm font-bold text-amber-600 mb-2 uppercase tracking-wider">
                    Coach Insight
                  </div>
                  <h3 className="text-lg font-black text-gray-900 mb-3">
                    {intelligence.actions[0].title}
                  </h3>
                ...
                <div className="relative z-10 text-left py-2">
                  <p className="text-gray-500 text-sm font-medium leading-relaxed">
                    Your personalized recommendations will appear after your first training session.
                    The more you practice, the smarter your preparation roadmap becomes.
                  </p>
                </div>
              )}
            </div>
