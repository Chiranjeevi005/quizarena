...
export function DynamicCTA({
  ...
  if (competition.status === "SCHEDULED") {
    return (
      <button
        disabled
        className="group relative inline-flex items-center justify-center gap-3 px-8 py-4 bg-slate-800 text-slate-400 font-bold rounded-xl cursor-not-allowed w-full"
      >
  ...
  if (!isEligible) {
    const isAttemptsExhausted = eligibilityDetails.some(
      (d) => d.requirement === "Attempts Remaining" && d.status === "NOT_ELIGIBLE"
    );
    ...
    return (
      <button
        disabled
        className="group relative inline-flex items-center justify-center gap-3 px-8 py-4 bg-slate-800/50 text-slate-500 font-bold rounded-xl cursor-not-allowed w-full"
      >
