...
export function CompetitionSnapshot({ competition }: CompetitionSnapshotProps) {
  ...
  const snapshotItems = [
    ...
    { label: "Passing Marks", value: config?.passingMarks ?? "N/A", icon: <CheckCircle2 /> },
    {
      ...
      value: config?.negativeMarkingEnabled ? `-${config.negativeMarkPerQuestion}` : "None",
      icon: <Hash />,
    },
    {
      label: "Attempts Allowed",
      value: config?.allowRetake ? "Multiple" : "1",
      icon: <CheckCircle2 />,
    },
    { label: "Language", value: competition.language.toUpperCase(), icon: <Languages /> },
    {
      label: "Randomization",
      value: config?.randomizeQuestions || config?.randomizeOptions ? "Yes" : "No",
      icon: <Shuffle />,
    },
  ];
    ...
    <section className="bg-slate-900 border border-slate-800 p-6 md:p-8 rounded-3xl shadow-xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {snapshotItems.map((item, idx) => (
          <div
            key={idx}
            className="bg-slate-950 p-4 rounded-xl border border-slate-800/50 flex flex-col items-start"
          >
            <div className="text-slate-500 mb-2 [&>svg]:w-5 [&>svg]:h-5">{item.icon}</div>
