...
export default async function OperationsDashboard({
  ...
  const getLifecycleState = (tab: string) => {
    switch (tab) {
      case "upcoming":
        return { in: [CompetitionLifecycle.READY, CompetitionLifecycle.SCHEDULED] };
      case "live":
        return { equals: CompetitionLifecycle.LIVE };
      case "completed":
        return { equals: CompetitionLifecycle.COMPLETED };
      case "archived":
        return { equals: CompetitionLifecycle.ARCHIVED };
      case "cancelled":
        return { equals: CompetitionLifecycle.CANCELLED };
      default:
        return { in: [CompetitionLifecycle.READY, CompetitionLifecycle.SCHEDULED] };
    }
  };
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Manage competition lifecycles, health, and emergencies.
          </p>
        </div>
      </div>
          ...
          <tbody>
            {competitions.map((comp) => (
              <tr key={comp.id} className="border-t">
                ...
                <td className="p-4">
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${comp.healthStatus === "CRITICAL" ? "bg-red-100 text-red-800" : comp.healthStatus === "WARNING" ? "bg-yellow-100 text-yellow-800" : "bg-green-100 text-green-800"}`}
                  >
                    {comp.healthStatus}
                  </span>
                </td>
                ...
                <td className="p-4">
                  {comp.startsAt ? new Date(comp.startsAt).toLocaleDateString() : "N/A"}
                </td>
                <td className="p-4">
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex gap-2">
          {page > 1 && (
            <Link
              href={`?tab=${tab}&page=${page - 1}${q ? `&q=${q}` : ""}`}
