...
class DraftController extends BaseController {
  public static async post(req: NextRequest) {
    return this.handleRequest(async () => {
      ...
      const result = await service.createDraft(questionId, userId);
      return result.getValue();
    });
  }

  public static async delete(req: NextRequest) {
    return this.handleRequest(async () => {
      const { questionId } = await req.json();
      const result = await service.discardDraft(questionId);
      return result.getValue();
    });
  }
}

export const POST = (req: NextRequest) => DraftController.post(req);
export const DELETE = (req: NextRequest) => DraftController.delete(req);
